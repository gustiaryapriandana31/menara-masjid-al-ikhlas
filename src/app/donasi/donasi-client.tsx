...
export default function KonfirmasiDonasiPage() {
  ...
  const showError = (message: string, title?: string) => {
    setAlertModal({ open: true, message, variant: "error", title })
  }
    ...
    if (e.target.files) {
      ...
      for (const file of filesArray) {
        if (file.size > 10 * 1024 * 1024) {
          showError(
            `Berkas "${file.name}" terlalu besar. Ukuran maksimal adalah 10MB. Silakan kompres gambar terlebih dahulu.`,
        ...
        if (!file.type.startsWith("image/")) {
          showError(
            `Berkas "${file.name}" bukan file gambar. Hanya JPG, PNG, dan WebP yang diperbolehkan.`,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Client-side validation → show modal
    if (!isAnonymous && !donorName.trim()) {
      showError("Silakan masukkan nama donatur.", "Nama Diperlukan")
      return
    }
    if (amount <= 0) {
      showError("Nominal donasi harus lebih dari Rp 0.", "Nominal Tidak Valid")
      return
    }
    if (!transferDate) {
      showError("Silakan tentukan tanggal transfer.", "Tanggal Diperlukan")
      return
    }
    if (!paymentChannel) {
      showError("Silakan pilih media/saluran pembayaran yang digunakan.", "Saluran Pembayaran")
      return
    }
    if (paymentChannel === "OTHER" && !customPaymentChannel.trim()) {
      showError("Silakan isi nama saluran transfer yang Anda gunakan.", "Nama Saluran Diperlukan")
      return
    }
    if (selectedFiles.length === 0) {
      showError("Silakan unggah minimal satu foto bukti transfer atau QRIS.", "Bukti Transfer Diperlukan")
      return
    }
    ...
    try {
      ...
      if (!result.success) {
        showError(
          result.error || "Gagal menyimpan konfirmasi donasi. Silakan coba lagi.",
          "Gagal Mengirim"
        )
        return
      }
      ...
      showError(
        err instanceof Error ? err.message : "Terjadi kesalahan jaringan. Periksa koneksi internet Anda dan coba lagi.",
        "Koneksi Bermasalah"
      )
    } finally {
